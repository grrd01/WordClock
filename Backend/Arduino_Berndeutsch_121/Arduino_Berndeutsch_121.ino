...
void loop() {
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            if (header.indexOf("snake") >= 0) {
              ...
              if (inSnake) {
                if (extractParameterValue(url, "dir=") > -2 && extractParameterValue(url, "dir=") < 5) {
  ...
  if (inSnake) {
    ...
    if (snakeWait > 0) {
      ...
      if (snakeDir == 1) {
        // move snake up
        Serial.println("inUp snakeNext:");
        snakeNext = snake[0] - 1 - 2 * snake[0] % 11;
        Serial.println(snakeNext);
        if (snakeNext < 0) {
          Serial.println("inTopBorder");
          snakeNext = -3;
        }
      } else if (snakeDir == 2) {
        // move snake right
      ...
      if (snakeNext == snake[1]) {
        ...
        Serial.println("ignoreBackwards");
        snakeNext = -2;
      }
      for (int i = snakeLen - 1; i > 1; i--) {
        if (snakeNext == snake[i]) {
          // bite own tail
          Serial.println("bite my tail");
          snakeNext = -3;
        }
      }
      if (snakeNext == snakeSnack) {
        // found snack
        Serial.println("found snack");
        snakeLen++;
        //todo: snack neu setzen
      }
